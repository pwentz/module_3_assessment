class BestBuyStoreSearchService
  ...
  def best_buy_store_search(search_query)
    response = @conn.get("https://api.bestbuy.com/v1/stores(area(#{search_query},25))?format=json&show=longName,city,distance,phone,storeType&pageSize=15&apiKey=#{ENV['best_buy_key']}")
